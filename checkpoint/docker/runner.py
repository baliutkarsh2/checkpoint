...
Wiring (one run):
  ...
  5. Create a host tempdir to use as /checkpoint-out (shared volume).
  6. Write a tmp /etc/hosts file: "127.0.0.1 api.github.com".
  7. Start sidecar container with /checkpoint-out + /etc/hosts mounted, exposing :443
     inside its netns, with --add-host host.docker.internal:host-gateway.
  8. Wait for /checkpoint-out/ca.crt to appear (sidecar's entrypoint minted it).
  9. Start harness container with network_mode=container:<sidecar>, same /checkpoint-out
     + /etc/hosts mounted, full env-var matrix injected.
  10. Wait for harness to exit (or timeout).
  11. Read /checkpoint-out/metrics.json and /checkpoint-out/agent-trace.json if present.
...
def _wait_for_ca(checkpoint_out: Path, timeout: float = 10.0) -> bool:
    deadline = time.time() + timeout
    cert = checkpoint_out / "ca.crt"
...
def _build_env(scenario: Scenario, judge_model: str) -> dict:
    env: dict = {
        # Checkpoint canonical vars (SCOPE §6.1)
        "CHECKPOINT_ENGINE_TASK": scenario.prompt,
        "CHECKPOINT_ENGINE_MODE": "docker",
        "CHECKPOINT_METRICS_FILE": "/checkpoint-out/metrics.json",
        "CHECKPOINT_AGENT_TRACE_FILE": "/checkpoint-out/agent-trace.json",
        "CHECKPOINT_OUT_DIR": "/checkpoint-out",
        # CA env vars (SCOPE §2.3 / §6.1)
        "NODE_EXTRA_CA_CERTS": "/checkpoint-out/ca.crt",
        "SSL_CERT_FILE": "/checkpoint-out/ca.crt",
        "REQUESTS_CA_BUNDLE": "/checkpoint-out/ca.crt",
        "CURL_CA_BUNDLE": "/checkpoint-out/ca.crt",
    ...
    if judge_model:
        env["CHECKPOINT_ENGINE_MODEL"] = judge_model
...
def _read_output(checkpoint_out: Path, name: str) -> Optional[dict]:
    p = Path(checkpoint_out) / name
    if not p.exists():
        return None
    try:
        return json.loads(p.read_text())
    except (json.JSONDecodeError, OSError):
        log.warning("malformed %s in /checkpoint-out; ignoring", name)
        return None
...
def docker_run_once(
    ...
    checkpoint_out = Path(tempfile.mkdtemp(prefix="checkpoint-checkpoint-out-"))
    sidecar = None
    harness = None
    ...
    try:
        ...
        hosts_file = _write_hosts_file(checkpoint_out)
        ...
        sidecar = client.containers.run(
            ...
            name=sidecar_name,
            volumes={
                str(checkpoint_out): {"bind": "/checkpoint-out", "mode": "rw"},
        ...
        if not _wait_for_ca(checkpoint_out):
        ...
        metrics = _read_output(checkpoint_out, "metrics.json")
        agent_trace = _read_output(checkpoint_out, "agent-trace.json")
        ...
        try:
            ...
            twin.kill()
        shutil.rmtree(checkpoint_out, ignore_errors=True)
        try:
