...
Tool names match Checkpoint's strict-mode tool list (SCOPE.md §3.5): 26
tools (24 live REST shims + 2 documented-as-stub tools). Each tool body
is a thin REST shim — REST and MCP share one `STATE` dict.

The two Checkpoint-documented stubs are `search_stripe_documentation` and
`stripe_integration_recommender`. We additionally ship
`send_stripe_mcp_feedback` as a no-op (Checkpoint lists it in §3.5 strict
mode and it's documented as a stub). All three return inert success
envelopes so MCP hosts that auto-introspect get the full Checkpoint
...
def build_mcp(app: FastAPI) -> FastMCP:
    ...
    mcp = FastMCP(
        ...
        instructions="Stateful synthetic Stripe (strict mode). Tool names match Checkpoint §3.5.",
        stateless_http=True,
    ...
    async def fetch_stripe_resources(limit: int = 10) -> Any:
        """Fetch arbitrary Stripe resources (Checkpoint docs this as stub-ish)."""
    ...
    async def search_stripe_documentation(query: str) -> Any:
        """Documented as a stub in Checkpoint §3.5. Returns an empty result list."""
        return {"data": [], "_stub": True, "query": query}

    @mcp.tool()
    async def stripe_integration_recommender(use_case: str | None = None) -> Any:
        """Documented as a stub in Checkpoint §3.5. Returns an empty list."""
    ...
    async def send_stripe_mcp_feedback(
        ...
        """Documented as a stub in Checkpoint §3.5. Always returns ok."""
