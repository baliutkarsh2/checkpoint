...
- Not a hosted SaaS.  No remote workspace, no `checkpoint_ws_*` tokens, no scenario marketplace.  We chose this deliberately — see [Security model](#15-security-model).
...
5. Waits for the sidecar to mint its CA cert into a shared volume (`/checkpoint-out/ca.crt`, plus `/checkpoint-out/bundle.pem` = public roots + that CA) and to print `ready` once its listeners are bound.
...
7. Starts the harness container on the bridge network with `extra_hosts: { "api.github.com": <sidecar_ip>, ... }`.  The harness's `entrypoint.sh` merges `/etc/ssl/certs/ca-certificates.crt` with `/checkpoint-out/ca.crt` into a combined CA bundle so OpenAI calls trust real CAs *and* sidecar-intercepted calls trust the minted CA.
