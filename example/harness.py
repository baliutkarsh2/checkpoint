...
TASK = os.environ.get("CHECKPOINT_ENGINE_TASK") or os.environ.get("CHECKPOINT_TASK")
MODEL = os.environ.get("CHECKPOINT_ENGINE_MODEL", "gpt-4o-mini")
...
def _write_outputs(final_text: str):
    try:
        ...
        log(f"[checkpoint-out write failed: {e}]")
...
def main():
    global LLM_CALLS
    if not TASK:
        print("Missing CHECKPOINT_ENGINE_TASK", file=sys.stderr)
        sys.exit(1)
