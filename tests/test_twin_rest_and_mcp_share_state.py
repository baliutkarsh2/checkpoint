"""One twin, two doors: REST and MCP write to the same state.

A twin serves its API twice — as HTTP for an SDK and as MCP for a tool-using
agent — and both have to be the same service. If they were two stores, an agent
that created an issue over MCP would be judged against a REST view that never
saw it, and the criterion would fail for a reason the agent could not have
avoided.

So for each of github, slack and stripe: call a mutating tool over MCP, read
``/_state`` over HTTP and find the change, then create a second resource over
REST and find both.
"""
