"""The `@checkpoint/vitest` package, checked from the Python suite.

Vitest cannot be run from here, but the ways this package breaks silently can
be: a `package.json` that no longer parses, an `index.js` that does not load,
or a type declaration that has drifted from what the module exports. A
JavaScript user hits all three as a confusing failure in their own suite.
