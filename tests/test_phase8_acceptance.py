...
QA-03: A scenario in Checkpoint's verbatim documentation shape (Title /
...
# --- QA-03: Checkpoint-verbatim scenario shape -----------------------------------

CHECKPOINT_HARNESS_SRC = '''
"""Tiny harness driving the Checkpoint-verbatim PR scenario.
...
HEADERS = {
    ...
    "User-Agent": "phase8-checkpoint-verbatim/0.1",
}
...
r.raise_for_status()
pr = r.json()
print(f"[checkpoint-harness] created PR #{pr['number']}", file=sys.stderr)
...
def test_qa03_checkpoint_verbatim_scenario_scores_green(mock_openai, tmp_path):
    """The Checkpoint-verbatim scenario runs against the github twin and scores >= 80."""
    scenario_path = SCENARIOS_DIR / "checkpoint-verbatim-github.md"
    assert scenario_path.is_file(), f"missing: {scenario_path}"

    harness = tmp_path / "harness.py"
    harness.write_text(CHECKPOINT_HARNESS_SRC)
    ...
    assert result.complete, f"run incomplete; stderr tail: {result.stderr[-500:]}"
    assert result.score >= 80.0, (
        f"checkpoint-verbatim scored {result.score}, expected >= 80\n"
        f"criteria: {[(c.text, c.passed, c.evaluator) for c in result.criteria]}"
    )
...
def test_qa03_verbatim_scenario_has_checkpoint_shape() -> None:
    """The verbatim scenario must use Checkpoint's exact section layout."""
    text = (SCENARIOS_DIR / "checkpoint-verbatim-github.md").read_text()
    for section in ("# ", "## Setup", "## Prompt", "## Success Criteria", "## Config"):
        assert section in text, f"missing section: {section}"
    # Must have at least one [D] and one [P] criterion (Checkpoint style).
    assert "[D]" in text
    assert "[P]" in text
