...
Minimum-viable clone of Checkpoint. Stateful SaaS twins + scenario runner + LLM judge.

## What works (v0)

- `checkpoint run scenario.md --harness "python harness.py"` end-to-end
- One twin: GitHub (issues, comments, labels, repos — REST subset)
- Scenario format: drop-in compatible with Checkpoint's markdown spec (Title, Setup, Prompt, Success Criteria with `[D]` / `[P]`, Config)
...
## What's deliberately missing vs Checkpoint
...
| `CHECKPOINT_ENGINE_TASK` | Alias for drop-in Checkpoint harness compatibility |
| `CHECKPOINT_ENGINE_MODE` | Always `"local"` |
