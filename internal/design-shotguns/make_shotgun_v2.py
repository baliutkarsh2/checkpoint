"""Shotgun v2 — 4 refined variants after first-round feedback.

Direction: keep A/B's warm palette (paper or dark-warm). Kill the 6px brutalist
offset shadow (that's what reads cartoonish). Replace with hairline 1px borders
and a single distinct identity marker per variant so it doesn't slide into
generic-Vercel territory.
"""
...
# ---------------------------------------------------------------------------
# I1 — Paper / Hairline / Accent Rail
# Paper bg, no shadows. Each card has a 2px electric-green left rail as the
# only chromatic accent. Distinct identity: every "object" has a green rail.
# ---------------------------------------------------------------------------
def v_paper_rail():
    rows = "".join(f"""
      <tr>
...
# ---------------------------------------------------------------------------
# I2 — Dark Warm / Hairline / Accent Rail
# Same treatment, dark warm palette.
# ---------------------------------------------------------------------------
def v_dark_rail():
    rows = "".join(f"""
      <tr>
...
.kpis{{display:grid;grid-template-columns:repeat(4,1fr);gap:0;margin-bottom:24px;
       border:1px solid #2a2a30;background:#161618;}}
...
# ---------------------------------------------------------------------------
# I3 — Paper / Typographic / No Chrome
# No card chrome at all. Sections separated only by full-width hairlines and
# generous whitespace. Identity comes from typography rhythm + mono micro-labels
# above each datum, NYT-data-page style. Distinctive without decoration.
# ---------------------------------------------------------------------------
def v_paper_typographic():
    rows = "".join(f"""
      <tr>
        <td class="m">{r['id'][:8]}</td>
        <td>{html.escape(r['scenario'])}</td>
        <td class="num" style="color:{ '#0ea83b' if r['score']>=100 else '#c89124' if r['score']>=50 else '#d73838'}">{int(r['score'])}</td>
        <td class="m">{r['crit_pass']}/{r['crit_total']}</td>
        <td class="m" style="color:#7d7568">{reltime(r['ts'])}</td>
      </tr>""" for r in SEED)
    return f"""<!doctype html><html><head><style>
@import url('https://fonts.googleapis.com/css2?family=Geist:wght@400;500;600;700&family=Geist+Mono:wght@400;500&display=swap');
*{{box-sizing:border-box;margin:0;padding:0;font-family:'Geist',sans-serif;}}
body{{background:#f5f2ea;color:#0a0a0a;font-size:13px;padding:28px;line-height:1.55;font-variant-numeric:tabular-nums;}}
.brand{{display:flex;align-items:baseline;gap:10px;font-weight:700;font-size:14.5px;margin-bottom:32px;}}
.mark{{width:11px;height:11px;background:#0a0a0a;position:relative;transform:translateY(1px);}}
.mark::after{{content:'';width:4.5px;height:4.5px;background:#2dff5c;position:absolute;top:3.25px;left:3.25px;}}
.kicker{{font-family:'Geist Mono',monospace;font-size:10px;text-transform:uppercase;letter-spacing:0.1em;color:#7d7568;margin-bottom:6px;}}
h1{{font-size:28px;font-weight:600;letter-spacing:-0.025em;margin-bottom:6px;line-height:1.05;}}
.sub{{color:#5a544c;font-size:13.5px;margin-bottom:36px;}}
.rule{{border-top:1px solid #0a0a0a;margin:0 0 18px;}}
.kpis{{display:grid;grid-template-columns:repeat(4,auto);gap:64px;margin-bottom:36px;}}
.k .kl{{font-family:'Geist Mono',monospace;font-size:9.5px;text-transform:uppercase;letter-spacing:0.1em;color:#7d7568;}}
.k .kv{{font-family:'Geist',sans-serif;font-size:34px;font-weight:600;line-height:1;margin-top:6px;letter-spacing:-0.03em;}}
.label{{font-family:'Geist Mono',monospace;font-size:10px;text-transform:uppercase;letter-spacing:0.1em;
        color:#7d7568;margin-bottom:10px;}}
table{{width:100%;border-collapse:collapse;}}
th{{font-family:'Geist Mono',monospace;font-size:9.5px;text-transform:uppercase;letter-spacing:0.1em;
   color:#7d7568;text-align:left;padding:8px 0;border-bottom:1px solid #0a0a0a;font-weight:500;}}
td{{padding:11px 0;border-bottom:1px solid #d6d0c5;font-size:13px;}}
td.m{{font-family:'Geist Mono',monospace;font-size:11.5px;}}
td.num{{font-family:'Geist',sans-serif;font-size:18px;font-weight:600;letter-spacing:-0.02em;}}
</style></head><body>
<div class="brand"><span class="mark"></span><span>checkpoint</span></div>
<div class="kicker">Recent activity</div>
<h1>Run history</h1>
<div class="sub">Fifteen runs across four scenarios. Four failures in the last seven days.</div>
<div class="rule"></div>
<div class="kpis">
  <div class="k"><div class="kl">Runs</div><div class="kv">15</div></div>
  <div class="k"><div class="kl">Average</div><div class="kv" style="color:#c89124">77.3</div></div>
  <div class="k"><div class="kl">Pass rate</div><div class="kv">73<span style="font-size:18px;color:#7d7568">%</span></div></div>
  <div class="k"><div class="kl">Fail · 7d</div><div class="kv" style="color:#d73838">4</div></div>
</div>
<div class="label">Latest runs</div>
...
# ---------------------------------------------------------------------------
# I4 — Dark Warm / Fused with Terminal / "Dense Mode"
# B's warmth + D's density. No shadows. Tight monospace data, sans headlines.
# Identity: every page is a tail -f vibe but with proper typography.
# Header has a faint green status pulse + run counter.
# ---------------------------------------------------------------------------
def v_dark_dense():
    rows = "".join(f"""
      <tr>
...
.bar .left .pulse{{width:6px;height:6px;background:#2dff5c;box-shadow:0 0 6px #2dff5c;border-radius:50%;
                   animation:p 1.5s infinite;}}
@keyframes p{{50%{{opacity:0.5;}}}}
.bar .right{{color:#8a8472;display:flex;gap:14px;}}
.bar .right b{{color:#ececf0;font-weight:500;}}
.brand{{font-family:'Geist',sans-serif;font-weight:700;font-size:14px;display:flex;align-items:center;gap:8px;
        padding:18px 24px 0;}}
...
h1{{font-family:'Geist',sans-serif;font-size:22px;font-weight:600;letter-spacing:-0.02em;
   padding:0 24px 4px;line-height:1.05;}}
.sub{{padding:0 24px 18px;color:#8a8472;font-size:11.5px;}}
.kpis{{display:grid;grid-template-columns:repeat(4,1fr);gap:0;margin:0 24px 18px;
       border:1px solid #2a2a30;}}
.k{{padding:11px 14px;border-right:1px solid #2a2a30;}}
.k:last-child{{border-right:none;}}
...
td{{padding:7px 14px;border-bottom:1px solid #1c1c1f;}}
...
<div class="bar">
  ...
  <div class="right"><span>15 runs</span><span>JUDGE <b>gpt-4o-mini</b></span><span>BUILD <b>v0.0.1</b></span></div>
</div>
<div class="brand"><span class="mark"></span><span>checkpoint</span></div>
<div class="kicker">Recent activity</div>
<h1>Run history</h1>
...
VARIANTS = [
    ("I1 · Paper · Accent Rail",
     "A's warmth, no shadows. Green 2px left-rail is the only chrome marker. Sober but distinctive.",
     v_paper_rail()),
    ("I2 · Dark Warm · Accent Rail",
     "B's warmth inverted, no shadows. Same green rail identity in dark mode.",
     v_dark_rail()),
    ("I3 · Paper · Typographic / No Chrome",
     "Zero card chrome. Sections are full-width hairlines + whitespace + typography only. Most editorial.",
     v_paper_typographic()),
    ("I4 · Dark Warm · Dense / Fused",
     "B + D fused. Mono body + sans headlines, util bar with live pulse, no card chrome. Engineer-credible.",
     v_dark_dense()),
]
...
<html><head><meta charset="utf-8"><title>checkpoint dashboard — shotgun v2</title>
<style>
*{{box-sizing:border-box;margin:0;padding:0;}}
...
<h1>checkpoint dashboard — shotgun v2</h1>
<p class="lede">Round 2 from feedback: keep A/B's warm palette family, kill the
brutalist offset shadow (cartoon vector), use hairline 1px borders. Each variant
carries one distinct identity marker so it doesn't read generic.</p>
<div class="grid">
{''.join(cards)}
</div>
</body></html>"""
...
def main():
    out = pathlib.Path("/tmp/shotgun-v2.html")
